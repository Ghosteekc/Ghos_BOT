...
class RecommendationEngine:
    ...
    def analyze(
        ...
        *,
        ...
        apply_swaps=True: arena/elixir prep + финальный trim на improved_deck.
        apply_swaps=False: тот же сценарий виртуально (для Passport / compare / stats).

        Одинаковая колода (+ контекст) → одинаковый план (RecommendationCache + tie-break).
        """
        ...
        # Единый последовательный сценарий (виртуальные замены).
        ...
        # Финальный Intent / GamePlan / Balance — по итоговой колоде сценария.
        ...
        # Balance issues для исходного анализа — по исходной колоде (до сценария),
        # чтобы UI видел стартовые проблемы; open_gaps — после сценария.
        start_balance = _balance_issues_for(original, db, arch)

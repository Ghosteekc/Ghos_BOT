"""Local counter database snapshot tiers are used without legacy counter overrides."""
...
def test_valkyrie_is_a_partial_local_counter_answer_to_golden_knight():
    assert card_counters_target("Valkyrie", "Golden Knight") == "partial"


def test_bats_keep_their_local_counter_partial_answer_to_golden_knight():
    assert card_counters_target("Bats", "Golden Knight") == "partial"


def test_minions_keep_their_local_counter_partial_answer_to_golden_knight():
    assert card_counters_target("Minions", "Golden Knight") == "partial"


def test_user_deck_has_partial_local_counter_answers_to_golden_knight():
    strong, partial = counters_in_deck("Golden Knight", USER_DECK)
    assert strong == []
    assert {"Valkyrie", "Bats"}.issubset(partial)


def test_golden_knight_is_flagged_dangerous_without_a_strong_local_counter_answer():
...
def test_golden_knight_answers_match_local_counter_tiers():
    expected = {
...
def test_bats_match_local_counter_tiers_against_splash_threats():
    expected = {
        "Mega Knight": None,
        "Boss Bandit": "partial",
        "Wizard": None,

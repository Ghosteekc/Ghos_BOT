...
def write_module(cards: dict[str, dict], all_slugs: list[str]) -> None:
    ...
    lines = [
        '"""',
        "Counter and synergy relationships scraped from deckshop.pro.",
        "",
        "Source: https://www.deckshop.pro/ru/",
        f"Scraped at (UTC): {scraped_at}",
        f"Cards in file: {len(cards)}",
        "",
        "NOT wired into bot logic yet — reference dataset for future matchup engine.",
        "",
        "Fields per card:",
        "  - counters_vs_attack: strong/partial cards that counter this card on offense",
        "  - counters_vs_defense: same for defensive role (if present on DeckShop)",
        "  - synergy_offense: cards that synergize when this card attacks",
        "  - not_killed_by_spells: spells that cannot fully kill this card alone",
        "  - weak_or_no_counter: cards NOT listed as counter (approx. anti-counter set)",
        "  - partial = DeckShop dimmed card (opacity-25), weaker counter/synergy",
        '"""',
        "",
        "from __future__ import annotations",
        "",
